...
[releases page](https://github.com/Velifag/Cut-Lab-IA-Studio/releases/latest).
...
An Intel (`macos-15-intel`) release leg was tried and dropped: Homebrew has no x86_64 Qt bottle for macOS 15, so `brew install qt` falls back to a from-source build that ran past a 2-hour job timeout without finishing.
